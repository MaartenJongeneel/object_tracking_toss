...
void MoveToStart::configure(const mc_rtc::Configuration & config)
{
}

void MoveToStart::start(mc_control::fsm::Controller & ctl_)
{
...
bool MoveToStart::run(mc_control::fsm::Controller & ctl_) // Called every iteration until it returns true
{
    auto & ctl = static_cast<ObjectTrackingToss &>(ctl_);
    
    auto io = mc_iam::IO::get(ctl.robot("ur10_with_sr_gripper"));

    if(!finishedUp && ctl.efTask_->eval().norm() > 0.001) {
        return false;
    }
    else {
...
void MoveToStart::teardown(mc_control::fsm::Controller & ctl_)
{
    auto & ctl = static_cast<ObjectTrackingToss &>(ctl_);
...
EXPORT_SINGLE_STATE("MoveToStart", MoveToStart)
